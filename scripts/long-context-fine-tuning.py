...
def parse_args():
    ...
    parser.add_argument('--prompt_tokens_target', type=int, help="Target number of tokens for the prompt")
...
def fine_tuning_main():
    ...
    train_dataset = create_long_context_synthetic_dataset(tokenizer, prompt_tokens_target=cfg.prompt_tokens_target)
...
def create_long_context_synthetic_dataset(tokenizer, prompt_tokens_target=1000):
    n_distractors = (prompt_tokens_target - 45)//18 + 1
    prompts = []
    for i in range(1000):
        user_message = f'Just reply with this number: {i:03d}. The following text is just a distraction.\n'
        for _ in range(n_distractors):
            user_message += "I'm just distracting you, don't forget the number!. Hehehe.\n"
        messages = [{"role": "system", "content": "You are a helpful assistant."},
                    {"role": "user", "content": user_message},
                    {"role": "assistant", "content": f'{i:03d}'}]
        prompt = tokenizer.apply_chat_template(
            messages, tokenize=False, add_generation_prompt=False)
        prompts.append(prompt)
    pretty_print_prompt(prompts[0])
    prompt_lengths = [len(tokenizer.tokenize(prompt)) for prompt in prompts]
    logger.info(f'Mean prompt tokens: {np.mean(prompt_lengths)}')
    logger.info(f'Max prompt tokens: {np.max(prompt_lengths)}')
    df = pd.DataFrame(dict(text=prompts))
